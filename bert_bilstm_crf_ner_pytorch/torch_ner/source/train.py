...
def train():
    ...
    if config.do_train:
        ...
        model.train()
        global_step, tr_loss, logger_loss, best_f1 = 0, 0.0, 0.0, 0.0
        for ep in trange(int(config.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_data_loader, desc="DataLoader")):
                ...
                if config.logging_steps > 0 and global_step % config.logging_steps == 0:
                    tr_loss_avg = (tr_loss - logger_loss) / config.logging_steps
            ...
            if config.do_eval:
                ...
                if f1_score > best_f1:
                    logger.info(f"******** the best f1 is {f1_score}, save model !!! ********")
                    best_f1 = f1_score
                    ...
                    logger.info("training_args.bin and ner_model.ckpt save successful!")
        writer.close()
